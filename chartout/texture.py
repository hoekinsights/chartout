...
def png_to_texture(*, png_data: bytes, product="403-11oz-color-mug", position: Position) -> bytes:
    ...
    img = Image.new("RGB", (config["area_width"], config["area_height"]), (255, 255, 255))
    ...
    chart_img = chart_img.resize((position["width"], position["height"]))  # Resize to fit the specified dimensions
...
def chart_to_texture(chart: Any, *, product="403-11oz-color-mug", position: Position) -> bytes:
    """Create a texture image from a chart."""
    return png_to_texture(png_data=chart_to_png(chart), product=product, position=position)
